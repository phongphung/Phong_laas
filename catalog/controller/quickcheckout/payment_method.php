...
class ControllerQuickCheckoutPaymentMethod extends Controller {
  	public function index() {
		...
		if (!empty($payment_address)) {
			...
			foreach ($results as $result) {
				if ($this->config->get($result['code'] . '_status')) {
					if($result['code']=='pp_standard'){
						$model_code='ppstandard';
					}
					else{
						$model_code=$result['code'];
					}
					$this->load->model('payment/' . $model_code);	
					////print_r('payment/' . $result['code']);
					
					
					$method = $this->{'model_payment_' . $model_code}->getMethod($payment_address, $total);
					
					if ($method) {

						if ($recurring) {
							if (method_exists($this->{'model_payment_' . $model_code}, 'recurringPayments') && $this->{'model_payment_' . $model_code}->recurringPayments()) {
