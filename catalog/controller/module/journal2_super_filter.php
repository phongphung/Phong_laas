...
class ControllerModuleJournal2SuperFilter extends Controller {
    ...
    private function parseUrl(){
        ...
        $fragments = explode("/", $this->request->post['filters']);
        foreach ($fragments as $fragment) {
            ...
            if (preg_match("/minPrice=/", $fragment)) {
                $minPrice = str_replace("minPrice=", "", $fragmet);
                continue;
            }
    ...
    public function products() {
        ...
        $url = '';
        $placeholder = $this->model_tool_image->resize(Front::$IS_OC2 ? 'placeholder.png' : 'no_image.jpg', $this->config->get('config_image_product_width'), $this->config->get('config_image_product_height'));
        ...
        foreach ($results as $result) {
          //echo "heee";
            if ($result['image']) {
                ...
                $image = $placeholder;//$this->model_tool_image->resize(Front::$IS_OC2 ? 'placeholder.png' : 'no_image.jpg', $this->config->get('config_image_product_width'), $this->config->get('config_image_product_height'));
            }
