...
function magiczoomplus_LoadScroll($tool, $profile = 'product') {
	...
	if($tool->params->checkValue('magicscroll', 'yes') && $tool->type == 'standard') {
		...
		$scroll->params->appendParams($tool->params->getParams());
		$GLOBALS["magictoolbox"]["scroll"] = & $tool;
		if($tool->params->checkValue('template', 'classic')) {
			$scroll->params->setValue('direction', 'right');
		}
		if($tool->params->checkValue('template', 'selectors-left')) {
			$scroll->params->setValue('direction', 'bottom');
		}
		return $scroll;
	}
	...
	$scroll->params->setScope('MagicScroll');
        static $scrollHeaders = '';
         if(!defined('MagicScrollModuleHeaders') && !defined('MAGICSCROLL_MODULE_HEADERS')) {
            $scrollHeaders = $scroll->getHeadersTemplate($prefix.'catalog/view/javascript',$prefix.'catalog/view/css');
        }
