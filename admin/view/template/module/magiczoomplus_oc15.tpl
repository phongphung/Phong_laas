...
function groupsHTML ($parameters, $profile = 'default', $map) {
    ...
    $groups = array();
        foreach ($parameters as $name => $s) {
			...
			if ($s['id'] == 'thumb-max-width' && !isset($s['value']) && in_array($profile,array('featured','bestseller','latest','specials'))) {
			    $s['default'] = '100';
			}
			
			if ($s['id'] == 'thumb-max-height' && !isset($s['value']) && in_array($profile,array('featured','bestseller','latest','specials'))) {
			    $s['default'] = '100';
			}
