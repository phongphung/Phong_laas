    ...
    function renderMultiLevelMenu($menu) {
        ...
        if (isset($menu['subcategories']) && is_array($menu['subcategories'])) {
            if (count($menu['subcategories']) > 0) {
              $html .= '<li class="hide-li-all-on-desktop">';
              if ($menu['href']) {
                  $html .= '<a href="' . $menu['href'] . '" ' . (isset($menu['target']) ? $menu['target'] : '') . '><span> Tat ca ' . $menu['name'] . '</span></a>';
              } else {
                  $html .= '<a><span>Tat ca ' . $menu['name'] . '</span></a>';
              }
              $html .= '</li>';
            }
            foreach ($menu['subcategories'] as $menu_item) {
                ...
                $html .= $submenu;
                if ($menu_item['href']) {
                    $html .= '<a class="'. (count($menu_item['subcategories']) > 0 ? 'trigger-on-mobile' : 'hide-link-no-submenu') .'">';
                    $html .= '<a href="' . $menu_item['href'] . '" ' . (isset($menu_item['target']) ? $menu_item['target'] : '') . ' class="'. (count($menu_item['subcategories']) > 0 ? 'hide-on-mobile' : '') .'">' . $menu_item['name'] . $span . '</a>';
                } else {
...
?>
  <!--  <div class="img-header" style="display:none">
        <a href="http://laas.vn"><img src="http://laas.vn/image/cache//data/logo-laas-80-80x49.png"></a>
    </div>
    -->
<div class="menu-sticker-wraper">
    ...
    <ul class="super-menu mobile-menu menu-<?php echo $display; ?>" style="table-layout: <?php echo $table_css_style; ?>">
        ...
        <li class="<?php echo $menu_item['type']; ?> <?php echo $menu_item['class']; ?>">
            ...
            <a <?php echo isset($menu_item['target']) ? $menu_item['target'] : ''; ?> 
               class="<?php echo count($menu_item['subcategories']) > 0 ? 'trigger-on-mobile' : 'hide-link-no-submenu'; ?>">
              <?php echo $menu_item['icon']; ?>
              <?php if (!$menu_item['hide_text']): ?>
                <span class="main-menu-text"><?php echo $menu_item['name']; ?></span>
              <?php endif; ?>
            </a>
            <a href="<?php echo $menu_item['href']; ?>" <?php echo isset($menu_item['target']) ? $menu_item['target'] : ''; ?> class="<?php echo count($menu_item['subcategories']) > 0 ? 'hide-on-mobile' : ''; ?>"><?php echo $menu_item['icon']; ?><?php if (!$menu_item['hide_text']): ?><span class="main-menu-text"><?php echo $menu_item['name']; ?></span><?php endif; ?></a>
            <?php else: ?>
            <a><?php echo $menu_item['icon']; ?><?php if (!$menu_item['hide_text']): ?>
              <span class="main-menu-text"><?php echo $menu_item['name']; ?></span>
              <?php endif; ?>
            </a>
            <?php endif; ?>
